...
class WIZARD(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y):
        super().__init__()
        self.sprites = []
        self.sprites.append(pygame.image.load(os.path.join('Assets', 'idle1.png')))
        self.sprites.append(pygame.image.load(os.path.join('Assets', 'idle2.png')))
        self.sprites.append(pygame.image.load(os.path.join('Assets', 'idle3.png')))
        self.sprites.append(pygame.image.load(os.path.join('Assets', 'idle4.png')))
        self.sprites.append(pygame.image.load(os.path.join('Assets', 'idle5.png')))
        self.sprites.append(pygame.image.load(os.path.join('Assets', 'idle6.png')))
...
def handle_missiles(missiles, gob):
    for shots in missiles:
        ...
        if pygame.sprite.spritecollideany(gob, shots):
            pygame.event.post(pygame.event.Event(GOB_HIT))
            missiles.remove(shots)
        if pygame.sprite.spritecollideany(BORDERONE, shots):
            pygame.event.post(pygame.event.Event(BORDER_HIT))
            missiles.remove(shots)
        if pygame.sprite.spritecollideany(BORDERTWO, shots):
            pygame.event.post(pygame.event.Event(BORDER_HIT))
            missiles.remove(shots)
        if pygame.sprite.spritecollideany(BORDERTHREE, shots):
            pygame.event.post(pygame.event.Event(BORDER_HIT))
            missiles.remove(shots)
        if pygame.sprite.spritecollideany(BORDERFOUR, shots):
            pygame.event.post(pygame.event.Event(BORDER_HIT))
            missiles.remove(shots)


def handle_spells(spells):
    for shots in spells:
        shots.x += MISSILEVELOCITY
...
def get_spell(cast, wiz):
    if cast == "jkl":
        stat = get_stat("hp")
        spellcast = Spell("Heal", stat, wiz.rect.x + wiz.rect.width, wiz.rect.y + wiz.rect.height//2)
        return spellcast
    elif cast == "jlk":
        stat = get_stat("hp") / 2
        spellcast = Spell("Cosmic Drain", stat, wiz.rect.x + wiz.rect.width, wiz.rect.y + wiz.rect.height//2)
        return spellcast
    elif cast == "kjl":
        stat = get_stat("ap") * 1.2
        spellcast = Spell("Sonic Blast", stat, wiz.rect.x + wiz.rect.width, wiz.rect.y + wiz.rect.height//2)
        return spellcast
    elif cast == "klj":
        stat = get_stat("ap") * 2
        spellcast = Spell("Fireball", stat, wiz.rect.x + wiz.rect.width, wiz.rect.y + wiz.rect.height//2)
        return spellcast
    elif cast == "ljk":
        stat = get_stat("df")
        spellcast = Spell("Shield", stat, wiz.rect.x + wiz.rect.width, wiz.rect.y + wiz.rect.height//2)
        return spellcast
    elif cast == "lkj":
        stat = get_stat("df") * 25
        spellcast = Spell("Thorns", stat, wiz.rect.x + wiz.rect.width, wiz.rect.y + wiz.rect.height//2)
        return spellcast
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE and len(missiles) < MAXINPUTS:
                ...
                if ''.join(spellStack) == 'klj':
                    spellCast = Spell('Fireball', 5, wizard.rect.x + wizard.rect.width, wizard.rect.y + wizard.rect.height // 2)
